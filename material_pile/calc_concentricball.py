...
class Page1:
    ...
    def show_concentric_spheres(self):
        ...
        r2 = (((r1 ** 3) / (
                    C_density / C_ratio * B_ratio / B_density + C_density / C_ratio * A_ratio / A_density + 1)) * (
                          C_density / C_ratio * B_ratio / B_density + 1)) ** (1 / 3)
        r3 = ((r1 ** 3) / (
                    C_density / C_ratio * B_ratio / B_density + C_density / C_ratio * A_ratio / A_density + 1)) ** (
                         1 / 3)
        # 定义球体的半径和颜色
        ...
        # 绘制同心球体
        for r, color, alpha in zip(radii, colors, alphas):
            # 创建球面
    ...
    def calculate_and_display(self):
        ...
        # 求出半径
        r1 = float(self.entries["A半径(外)"].get())
        r2 = (((r1**3)/(C_density/C_ratio*B_ratio/B_density+C_density/C_ratio*A_ratio/A_density+1))*(C_density/C_ratio*B_ratio/B_density+1))**(1/3)
        r3 = ((r1**3)/(C_density/C_ratio*B_ratio/B_density+C_density/C_ratio*A_ratio/A_density+1))**(1/3)
        ...
        # 判断小球半径的2倍是否大于盒子大小
            if diameter1 > box_size_length:
                raise ValueError("Abnormal small ball radius: The diameter of the big ball is larger than the size of the box")

            if diameter2 > diameter1:
                raise ValueError("Abnormal small ball radius: The diameter of the middle ball is larger than the size of the big ball")

            if diameter3 > diameter2:
                raise ValueError("Abnormal small ball radius: The diameter of the small ball is larger than the size of the middle ball")
        ...
        # 面心立方堆积（FCC）填充密度约为74%
        def calculate_fcc_positions(L, r):
            positions = []

            # 基本单元格的边长 (等于 2 * sqrt(2) * r)
            unit_length = 2 * np.sqrt(2) * r
            ...
            # 计算FCC堆积中的所有球心位置
            i = 0
            while i * unit_length + r <= L:
                j = 0
                while j * unit_length + r <= L:
                    k = 0
                    while k * unit_length + r <= L:
                        for (x_frac, y_frac, z_frac) in unit_positions:
                            x = (i + x_frac) * unit_length + r
                            y = (j + y_frac) * unit_length + r
                            z = (k + z_frac) * unit_length + r
                            # 确保球完全在立方体内部
        ...
        positions = calculate_fcc_positions(box_size_length, r1)    # 默认是立方体
        positions_num = len(positions)
        # 每个球与12个球接触
        contact_counts = int(positions_num*12/2)
        ...
        # 计算每类材料体积
        volume_A = num_A * (4 / 3) * np.pi * (r1 ** 3 - r2 ** 3)
        volume_B = num_A * (4 / 3) * np.pi * (r2 ** 3 - r3 ** 3)
        ...
        # 计算每类材料的质量
        mass_A = num_A * (4 / 3) * np.pi * (r1 ** 3 - r2 ** 3) * A_density
        mass_B = num_A * (4 / 3) * np.pi * (r2 ** 3 - r3 ** 3) * B_density
        ...
        B_C_area = 4 * np.pi * (r3 ** 2) * num_A

        result_text += f"Number of A_A tangent pairs: {A_A_area}\n"
        result_text += f"ALL of area A-B pairs: {A_B_area}\n"
        print(f"ALL of area A-B pairs: {A_B_area}")
        result_text += f"ALL of area B-C pairs: {B_C_area}\n"
        print(f"ALL of area B-C pairs: {B_C_area}")
        ...
        self.entries["B-C接触面积"].insert(0, B_C_area)
        ...
        for pos in positions:
            ...
            zs = z + r1 * np.cos(v)
            ax.plot_surface(xs, ys, zs, color=colors['A'], alpha=0.6)
...
def main():
    root = tk.Tk()
    root.title("circular_concentric_ball")
